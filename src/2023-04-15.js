const { distance, loadImg } = require("./tools");
...
const src = loadImg("flowers1.png");
const avgData = avgImgData();
const { width, height } = avgData;

const points = new Array(200).fill(0).map((_, i) => ({
  x: 700 + 400 * Math.cos((Math.PI * i) / 100),
  y: 700 + 400 * Math.sin((Math.PI * i) / 100)
}));
