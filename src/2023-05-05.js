...
const images = new Array(36).fill(0).map((_, i) => {
  const xo = 700 + 400 * Math.sin((Math.PI * i) / 18);
  const yo = 700 + 400 * Math.cos((Math.PI * i) / 18);
  const img = emptyData();
...
buildAnimation({ images, delay: 100 }).writeOut(__filename);
